...
- 关键写操作（AI 回写、自动记账生成、周期账单、收款/还款）必须幂等。
...
- 交付物：
  ...
  - 收款/还款/结清（减少可收回/需归还余额，余额 0 结清，不改原交易有效金额）。
- 依赖：B3。
...
### F5 — 账户
- 交付物：`/app/:ledgerId/accounts`（净资产 + 分组）、账户/子账户详情、余额调整、收款/还款/结清、账户流水、关联记录。
- 依赖：F2、B4。
- 验收：余额展示以后端为准；调整/收还款后失效相关缓存。
