...
import { DeleteLedgerConfirmDialog } from "./DeleteLedgerConfirmDialog";
...
export function LedgerDetailSheet({ ledgerId }: { ledgerId: string }) {
  ...
  const [ledgerPendingDelete, setLedgerPendingDelete] = useState(false);
  ...
  const whiteButtonClass = "!bg-[var(--color-bg-surface)]";
  ...
  return (
    <>
        ...
        <div className="grid grid-cols-[var(--space-control-height)_1fr_var(--space-control-height)] items-center gap-3">
          ...
          {isOwner ? (
            <IconButton
              ...
              label="编辑账本"
              onClick={() =>
                push({ hideDefaultHeader: true, content: <CreateLedgerSheet ledger={ledger} /> })
              }
            />
          ) : (
            <span aria-hidden />
        ...
        {isOwner ? (
          <div className="flex flex-col gap-2">
            <Button
              ...
              onClick={() => setLedgerPendingDelete(true)}
              variant="danger"
            >
              删除账本
            </Button>
      ...
      <DeleteLedgerConfirmDialog
        deleting={deleteLedger.isPending}
        ledger={ledgerPendingDelete ? ledger : null}
        onCancel={() => {
          if (!deleteLedger.isPending) setLedgerPendingDelete(false);
        }}
        onConfirm={() => {
          if (!deleteLedger.isPending) deleteLedger.mutate();
        }}
      />
    </>
  );
