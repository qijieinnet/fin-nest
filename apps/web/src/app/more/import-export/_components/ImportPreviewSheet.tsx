  ...
  return (
    <div className="flex flex-col gap-4 pb-2">
      <div className="grid grid-cols-[var(--space-control-height)_1fr_var(--space-control-height)] items-center gap-3">
        ...
        <h2 className="text-center text-base font-semibold text-[var(--color-text-primary)]">导入预览</h2>
        <IconButton
          disabled={!canCommit}
      ...
      <section className="overflow-hidden rounded-[14px] bg-[var(--color-bg-surface)] shadow-[var(--shadow-soft)]">
          ...
          {IMPORT_COUNT_LABELS.filter(([key]) => result.counts[key]).map(([key, label]) => {
            ...
            return (
              <li className="flex items-center px-4 py-2.5" key={key}>
                <span className="min-w-0 flex-1 text-sm text-[var(--color-text-primary)]">{label}</span>
                <span className="shrink-0 text-[13px] text-[var(--color-text-muted)]">{parts.join(" · ")}</span>
              </li>
            );
          })}
        </ul>
      ...
      {totalNew === 0 && !hasErrors ? (
        <p className="text-sm text-[var(--color-text-muted)]">没有识别到新增行（新增行请将 ID 列留空）。</p>
      ) : null}

      <IssueList issues={result.errors} title={`错误（${result.errors.length}），修正后请重新导入`} tone="error" />
      <IssueList issues={result.warnings} title={`提示（${result.warnings.length}）`} tone="warning" />
      ...
      {commit.isPending ? <p className="text-sm text-[var(--color-text-muted)]">正在导入…</p> : null}
    </div>
  );
}

function IssueList({ issues, title, tone }: { issues: ImportRowIssue[]; title: string; tone: "error" | "warning" }) {
  if (issues.length === 0) return null;
  const color = tone === "error" ? "text-[var(--color-accent-expense)]" : "text-[var(--color-text-secondary)]";
  return (
    <section>
      <p className={`text-sm font-semibold ${color}`}>{title}</p>
      <ul className="mt-1.5 max-h-48 overflow-y-auto rounded-[12px] bg-[var(--color-bg-surface)] px-3 py-2 shadow-[var(--shadow-soft)]">
