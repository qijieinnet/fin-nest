...
import {
  ACCOUNT_COLUMNS,
  ACCOUNT_ENTRY_COLUMNS,
  ACCOUNT_ENTRY_TYPE_LABELS,
  ACCOUNT_TYPE_LABELS,
...
type LedgerData = Awaited<ReturnType<ExcelWorkbookService["loadLedgerData"]>>;

/**
 * 账本 Excel 工作簿构建。**不做鉴权**——调用方负责：
 * api 侧 `ExcelExportService` 先 `assertMember` 再委托，worker 侧系统备份按账本直接生成。
 */
@Injectable()
export class ExcelWorkbookService {
  ...
  async buildWorkbook(
    ...
    const data = await this.loadLedgerData(ledgerId, options.template, client);
    ...
    if (!options.template) {
      ...
      this.addAccountEntriesSheet(workbook, data);
    }
    if (options.template) {
  ...
  private async loadLedgerData(
    ...
    // 账户流水与记账人只用于全量导出：模板不含数据行，白拉一遍大表没意义。
    const accountEntries = template
      ? []
      : await client.accountEntry.findMany({ where, orderBy: { occurredAt: "asc" } });
    const creatorIds = template
      ? []
      : Array.from(new Set(transactions.map((row) => row.createdBy).filter(Boolean)));
  ...
  private addTransactionsSheet(
    ...
    const sheet = this.addSheet(
      ...
      template ? TRANSACTION_COLUMNS : [...TRANSACTION_COLUMNS, ...TRANSACTION_EXPORT_COLUMNS],
    );
    ...
    for (const transaction of data.transactions) {
      ...
      const row = sheet.addRow({
        ...
        createdByName: transaction.createdBy
          ? (data.creatorById.get(transaction.createdBy)?.alias ?? "")
          : "",
        createdAt: dateTimeToText(transaction.createdAt),
      });
  ...
  /** 账户流水（只导出）。含余额调整与冲正，脱离系统后靠它复核每个账户的余额来龙去脉。 */
  private addAccountEntriesSheet(workbook: ExcelJS.Workbook, data: LedgerData): void {
    const sheet = this.addSheet(workbook, SHEET_NAMES.accountEntries, ACCOUNT_ENTRY_COLUMNS);
    for (const entry of data.accountEntries) {
      const row = sheet.addRow({
        occurredAt: dateTimeToText(entry.occurredAt),
        account: data.accountById.get(entry.accountId)?.name ?? "",
        subAccount: entry.subAccountId
          ? (data.subAccountById.get(entry.subAccountId)?.name ?? "")
          : "",
        entryType: labelOf(ACCOUNT_ENTRY_TYPE_LABELS, entry.entryType),
        amountDelta: microsToYuanNumber(entry.amountDeltaMicros),
        balanceBefore: microsToYuanNumber(entry.balanceBeforeMicros),
        balanceAfter: microsToYuanNumber(entry.balanceAfterMicros),
        transactionId: entry.transactionId ?? "",
        note: entry.note ?? "",
      });
      for (const key of ["amountDelta", "balanceBefore", "balanceAfter"]) {
